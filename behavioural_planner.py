...
class BehaviouralPlanner:
    ...
    def transition_state(self, waypoints, ego_state, closed_loop_speed, co_action, is_new_action=False):
        """Handles state transitions and computes the goal state.
        ...
        # understand it.
        # action = 0
        # if co_action in [0, 4, 5]:
        #     action = 0
        # if co_action in [1, 2, 3]:
        #     action = 1
        # if co_action in [6, 7, 8]:
        #     action = 2
        # action = co_action
        if is_new_action:
            self.trans_reward = 0.0
        if self._state == FOLLOW_LANE:
            ...
            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
            # ------------------------------------------------------------------
            # if action == 1 and is_new_action:
            if stop_sign_found or self._goal_index == len(waypoints) - 1:
                self._stopsign_index = goal_index
                self._goal_state[2] = 0.0
                self._state = DECELERATE
                # print("Switch state to DECELERATE")
                # self.trans_reward = -0.05
            # if action == 2 and is_new_action:
            #     self.trans_reward = -1.0
            # ------------------------------------------------------------------
        ...
        # the stop sign and transition to the next state.
            ...
            # if action == 1 and is_new_action:
            #     self.trans_reward = -1.0
            if self._stop_count == STOP_COUNTS or is_new_action:
                self._stop_count = 0
                # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                # --------------------------------------------------------------
                closest_len, closest_index = get_closest_index(waypoints, ego_state)
                ...
                # --------------------------------------------------------------
                # if action == 0 and is_new_action:
                if not stop_sign_found and self._goal_index != len(waypoints) - 1:
                    self._state = FOLLOW_LANE
                    # print("Switch state to FOLLOW LANE")
                    # self.trans_reward = -0.1
                # --------------------------------------------------------------

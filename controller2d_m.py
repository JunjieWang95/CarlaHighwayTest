...
class PIDLateralController():
    ...
    def __init__(self, K_P=1.0, K_D=0.0, K_I=1.0, dt=0.05):
        """
    ...
    def pid_control(self, v_vec, w_vec):
...
class Controller2D(object):
    def __init__(self, waypoints):
        ...
        ## Pure Pursuit
        self.pp                  = PP(L=4.5, k=1.3, k_Ld=1.3)

        self.lateral_pid = PIDLateralController()
    ...
    def update_controls(self):
        ...
        if self._start_control_loop:
            ...
            self.throttle_brake_pid.update(speed_err, output_limits = [-0.2, 0.7])            
            ...
            if v < eps:
                ...
                alpha = np.arctan2(waypoints[int(2*len(waypoints)/3.)][1]-y-front_distance*np.sin(yaw),
                                   waypoints[int(2*len(waypoints)/3.)][0]-x-front_distance*np.cos(yaw))
